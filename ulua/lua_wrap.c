...
#   pragma intrinsic(__rdtsc)
	...
	void __cpuid(unsigned int inf[4], unsigned int type);
...
void cpuid(unsigned int inf[4], unsigned int type)
{
	return __cpuid(inf, type);
}

unsigned long _freq(int test_times)
{
	double fac = 0.0;
	...
	{
		unsigned long start = counter();
		unsigned long pcstart;
		QueryPerformanceCounter((LARGE_INTEGER*)&pcstart);
		Sleep(20);
		unsigned long pcstop;
		QueryPerformanceCounter((LARGE_INTEGER*)&pcstop);
		unsigned long stop = counter();

		unsigned long c = stop - start;
		unsigned long pc = pcstop - pcstart;
	...
	unsigned long cpufreq = count*pfreq / pcount;
	return cpufreq;
}


unsigned long ULUATool_cpu_freq()
{
	unsigned long u = _freq(4);
	return u;
}
#else
pg_noinline __declspec(naked) uint64 counter()
{
	__asm
	{
		rdtsc
...
__declspec(naked) void cpuid(uint[4], uint)
{
	__asm
	{
		push ebp
